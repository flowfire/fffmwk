...
module.exports = async ({ request, body, config }) => {
    ...
    if (urlArr.length !== 0 && urlArr[0] === "api") {
        // api
        Object.assign(response.headers, config.apiHeaders);
        ...
        let real_return;
        if (config.timeout) {
            real_return = await Promise.race([
                new Promise(res => {
                    if (temp_return instanceof Promise) {
                        temp_return.then(response => res(response));
                    } else {
                        res(temp_return);
                    }
                }),
            ...
            else real_return = await temp_return;
        }
        ...
        // statics
        Object.assign(response.headers, config.staticHeaders);
